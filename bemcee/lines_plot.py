...
from .utils import beta, geneva_interp_fast, griddataBAtlas, griddataBA, linfit, jy2cgs, check_list
import corner
...
def plot_residuals_new(par, npy, current_folder, fig_name):
    '''
    ...
    #if flag.model == 'aeri':
    #    if check_list(info.lista_obs, 'UV'):
    #
    ...
    #        
    #    if flag.binary_star:
    #            logF_mod_UV_1 = griddataBA(info.minfo, info.logF_grid[index], par[:-lim], info.listpar, info.dims)
    #            logF_mod_UV_2 = griddataBA(info.minfo, info.logF_grid[index], np.array([M2, 0.1, par[2], par[3]]), info.listpar, info.dims)
    #            logF_mod_UV = np.log10(10.**logF_mod_UV_1 + 10**logF_mod_UV_2)
    #        
    #    else:
    #        logF_mod_UV = griddataBA(info.minfo, info.logF_grid[index], par[:-lim], info.listpar, info.dims)
    #    
    #
    #    # convert to physical units
    #    logF_mod_UV += np.log10(norma)
    ...
    #    for i in range(len(par_list)):
    #        if flag.binary_star:
    #            logF_mod_UV_1_list = griddataBA(info.minfo, info.logF_grid[index], par_list[i, :-lim], info.listpar, info.dims)
    #            logF_mod_UV_2_list = griddataBA(info.minfo, info.logF_grid[index], np.array([par_list[i, -1], 0.1, par_list[i, 2], par_list[i, 3]]), info.listpar, info.dims)
    #            logF_list[i] = np.log10(10.**np.array(logF_mod_UV_1_list) + 10.**np.array(logF_mod_UV_2_list))
    #        else:
    #            logF_list[i] = griddataBA(info.minfo, info.logF_grid[index], par_list[i, :-lim],
    #                                  info.listpar, info.dims)
    #    
    #    
    #    logF_list += np.log10(norma)
...
def plot_line(line, par, par_list, current_folder, fig_name):
    ...
    #if flag.binary_star:
    #    F_mod_line_1 = griddataBA(info.minfo, info.logF_grid[index], par[:-lim], info.listpar, info.dims)
    #    F_mod_line_2 = griddataBA(info.minfo, info.logF_grid[index], np.array([par[-1], 0.1, par[2], par[3]]), info.listpar, info.dims)
    ...
    #else:
    #    F_mod_line_unnorm = griddataBA(info.minfo, info.logF_grid[index], par[:-lim], info.listpar, info.dims)
    #    flux_mod_line = linfit(info.wave[index], F_mod_line_unnorm)
