...
def plot_residuals(par, npy, current_folder, fig_name):
    ...
    if flag.pol:
        ...
        for i, params in enumerate(par_list):
            logF_list[i] = griddataBA(info.minfo, info.logF_grid[index], params,
                              info.listpar, info.dims)
        ...
        ax2.plot(lbd_pol, (flux - logF_list[-1]) / sigma, 'ks', ms = 5, alpha=0.2)
        ax1.errorbar(lbd_pol, flux, yerr=sigma, ls='', marker='o',
                     alpha=0.5, ms=5, color='k', linewidth=1)
        ...
        plt.tight_layout()
        plt.savefig(current_folder + fig_name + '_new_residuals-POL' + '.png', dpi=100)
        plt.close()
...
def plot_line(line, par, par_list, current_folder, fig_name):
    ...
    ax2.plot(vl, (fxx - fx)/dflux_line, marker='o', color='k', alpha=0.5)
