...
from bemcee.utils import beta, geneva_interp_fast, griddataBAtlas, griddataBA, lineProf, linfit
from bemcee.lines_reading import create_list, read_BAphot2_xdr, read_observables, create_tag, read_acol_Ha_xdr
import corner
...
dist_pc, sig_dist_pc, vsin_obs, sig_vsin_obs, file_ebmv, file_incl, file_dincl = read_stars(list_of_stars)
file_plx, file_dplx, file_vsini, file_dvsini = dist_pc, sig_dist_pc, vsin_obs, sig_vsin_obs
ranges, Ndim = set_ranges(star, lista_obs, listpar)
...
if flag.model == 'beatlas':
    new_ranges[1] = obl2W(ranges[1])
    new_ranges[4] = (np.arccos(ranges[4])) * (180. / np.pi)
...
def plot_residuals_new(par, lbd, logF, dlogF, minfo, listpar, lbdarr, logF_grid,
    ...
    if flag.SED:
        ...
        for i, params in enumerate(par_list):
            if flag.binary_star:
                logF_mod_UV_1_list = griddataBA(minfo, logF_grid[index], params[:-lim], listpar, dims)
                logF_mod_UV_2_list = griddataBA(minfo, logF_grid[index], np.array([params[-1], 0.1, params[2], params[3]]), info.listpar, info.dims)
                logF_list[i] = np.log10(10.**np.array(logF_mod_UV_1_list) + 10.**np.array(logF_mod_UV_2_list))
            else:
                if flag.model != 'beatlas':
                    logF_list[i] = griddataBA(minfo, logF_grid[index], params[:-lim],
                                      listpar, dims)
                else:
                    logF_list[i] = griddataBAtlas(minfo, logF_grid[index], params[:-lim],
                                      listpar, dims, isig = dims["sig0"])
            #par_list[i] = params
        ...
        ax1.errorbar(lbd_UV[keep], flux_UV[keep], yerr=dflux[keep], ls='', marker='o',
                     alpha=0.5, ms=4, color='k', linewidth=1)
...
def plot_line(line, lista_obs, minfo, F_grid, par, listpar, dims, flux, errors, lbd, par_list, current_folder, fig_name):
   ...
   #     flux_mod_line = linfit(lbd[index], F_mod_line_unnorm)
    ...
    for i, params in enumerate(par_list):
        ...
        if flag.binary_star:
            F_mod_line_1_list = griddataBA(minfo, F_grid[index], params[:-lim], listpar, dims)
            F_mod_line_2_list = griddataBA(minfo, F_grid[index], np.array([params[-1], 0.1, par_list[i, 2], par_list[i, 3]]), listpar, dims)
            F_list[i]  = linfit(lbd[index], F_mod_line_1_list + F_mod_line_2_list)
            #logF_list[i] = np.log(norm_spectra(lbd[index], F_list))
        else:
            F_list_unnorm[i] = griddataBA(minfo, F_grid[index], params[:-lim], listpar, dims)
    ...
    ax2.errorbar(vel[keep], flux_line[keep], yerr= dflux_line[keep], ls='', marker='o', alpha=0.5, ms=4, color='blue', linewidth=1)
...
corner(samples, labels=labels, labels2=labels, range=new_ranges, quantiles=None, plot_contours=True, show_titles=False,
...
fig, ax = corner(samples, labels=labels, range=ranges, quantiles=None, truths=None,
    ...
    for i in range(len(ax)):
        ...
        if i <= len(ax):
            for j in np.arange(i+1, len(ax.T)):
                ...
                # poly
                hist2d(newsamps[:, i], newsamps[:, j],  smooth=1,
